							...
							<c:when test="${general.size()!=0}">
								<c:forEach var="g" items="${general}">
									<div class="list_n" style="background-color: white;">
										<div>${g.writer}</div>
										<div id="t">
											<c:set var="title" value="${g.title}" />
											<c:if test="${fn:length(title)>16}">
												<c:set var="title" value="${fn:substring(title,0,16)}..." />
											</c:if>
											<c:if test="${g.reply_count>0}">
												<c:set var="title" value="${title} (${g.reply_count})" />
											</c:if>
											<a title="${g.title}"
												href="/board/read?post_num=${g.post_num}">${title} </a>
										</div>
										<div>${g.heart_count}</div>
										<div>${g.view_count}</div>
									</div>
								</c:forEach>
							</c:when>
							<c:otherwise>
